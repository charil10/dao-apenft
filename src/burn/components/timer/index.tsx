...
const Timer = (props: PropsWithChildren<TagProps>) => {
  ...
  return (
    <div className="ui-timer" {...rest}>
      <span className="timer-item f-dinpro">{`${config.h}`.padStart(2, '0')} HOURS</span>
      <span className="timer-item f-dinpro">{`${config.m}`.padStart(2, '0')} MINS</span>
      <span className="timer-item f-dinpro">{`${config.s}`.padStart(2, '0')} SECS</span>
    </div>
  )
}
